...
public class BlockBlastPanel extends MinigamePanel {
    ...
    private void initGame(long timeLimit, int target, int difficulty) {
        ...
        // Tìm đến phần khởi tạo giveUpBtn trong initGame
        // 4. XỬ LÝ SỰ KIỆN KHI BẤM NÚT GIVE UP
        giveUpBtn.addActionListener(e -> {
            // Chỉ đánh dấu kết thúc, không thoát ngay
            this.isGameOver = true;
            this.isVictory = false;

            stopTimers(); // Dừng rơi gạch
            // stopMusic(); // Có thể để nhạc chạy tiếp hoặc dừng tùy ông

            // Ẩn nút Give Up đi để người chơi không bấm đè lên bảng thông báo
            giveUpBtn.setVisible(false);

            // Ép vẽ lại để hiện cái Overlay "MISSION FAILED"
            repaint();
        });
        add(giveUpBtn);
