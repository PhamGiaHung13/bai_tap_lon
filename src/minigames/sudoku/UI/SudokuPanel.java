...
public class SudokuPanel extends MinigamePanel {
    ...
    public SudokuPanel(int difficulty, GameController controller) {
        ...
        MouseAdapter clickToExit = new MouseAdapter() {
            @Override
        ...
        // 1. Gán cho chính SudokuPanel
        this.addMouseListener(clickToExit);

        // 2. Duyệt qua tất cả các thành phần con để gán listener
        // Việc này đảm bảo ông click vào header, bảng Sudoku hay các nút số đều thoát được
        for (Component c : getComponents()) {
            c.addMouseListener(clickToExit);
            if (c instanceof Container) {
                // Duyệt sâu thêm một tầng nữa (vào trong các bảng con)
                for (Component sub : ((Container) c).getComponents()) {
                    sub.addMouseListener(clickToExit);
                    if (sub instanceof Container) {
                        for (Component subSub : ((Container) sub).getComponents()) {
                            subSub.addMouseListener(clickToExit);
                        }
                    }
                }
            }
